...
def test_register_map_and_access() -> None:
    ...
    for name in ("power", "setpoint", "hvac_mode", "fan_speed"):
        assert fields[name].writable is True
    for name in (
        "room_temperature", "fan_percentage", "louver", "available_modes",
        "available_speeds", "slave_address", "baud_configuration", "parity_configuration",
    ):
...
def test_verified_values() -> None:
    assert DaikinAidoo.declared_fields["power"].decode([1]) is True
